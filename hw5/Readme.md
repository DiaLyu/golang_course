# Домашнее задание №5: веб-фреймворк на основе кодогенерации

## Задача

Необходимо реализовать кодогенератор (handlers_gen/codegen.go), который: 
1. Парсит переданный Go-файл (например, api.go) 
2. Находит методы структуры, помеченные комментарием: 
```
// apigen:api {"url": "...", "method": "...", "auth": true}
``` 
3. Генерирует файл (например, api_handlers.go) со следующим функционалом:
    - HTTP-обёртки для методов 
    - Проверка HTTP-метода (GET/POST) 
    - Проверка авторизации (X-Auth: 100500) 
    - Валидация параметров по struct tags apivalidator 
    - Заполнение структуры параметров 
    - Обработка ошибок (включая неизвестные через ApiError) 
    - Формирование JSON-ответа 
    - Генерация ServeHTTP 

⚠ Важное требование: Никакого хардкода имён структур, полей или значений. Всё извлекается из AST.
...
> go build -o codegen.exe ./handlers_gen
...
Используются go/parser, go/ast, go/token.

**Обрабатываются методы:**

```
node.Decls -> *ast.FuncDecl 
```

Отбираются только те, у которых есть:
```
apigen:api {JSON}
```

Извлекаются:
- receiver (тип структуры)
- имя метода
- аргументы
- JSON-настройки (url, method, auth)

**Структуры извлекаются через:**
```
*ast.GenDecl -> *ast.TypeSpec -> *ast.StructType 
```
Структура параметров берется из второго аргумента метода:
```
func (h *MyApi) Create(ctx context.Context, params CreateParams)
```

### 2. Обработка настроек apigen:api

JSON десериализуется в:
```
...
### 3. Разбор apivalidator
Поддерживаемые теги:
| Тег       | Поведение                          |
| --------- | ---------------------------------- |
| required  | значение не должно быть zero-value |
| paramname | имя HTTP-параметра                 |
| enum      | допустимые значения                |
| default   | значение по умолчанию              |
| min       | ≥ X                                |
| max       | ≤ X                                |

Поддерживаемые типы: int, string

## Генерируемая структура кода
...
**respondError**
- Если ошибка - ApiError, используется его HTTPStatus
- Иначе - 500

В реализации используется рекурсивный вызов для unknown error: 
```
return respondError(w, ApiError{...}) 
```

**wrapper-метод**
Для каждого API-метода генерируется: 
```
func (h *MyApi) handleCreate(w http.ResponseWriter, req *http.Request) 
```
Внутри: 
1. Проверка авторизации (если требуется) 
2. Создание params 
3. Заполнение полей через req.FormValue 
4. Применение default 
5. Проверка required 
6. enum 
7. min 
8. max 
9. Вызов бизнес-метода 
10. Обработка ошибки 
11. Формирование JSON-ответа 

Порядок валидации полей соответствует порядку их объявления в структуре (как требует ТЗ).

**ServeHTTP** 
Генерируется один метод на структуру: 
```
func (h *MyApi) ServeHTTP(w http.ResponseWriter, req *http.Request) 
```
Логика: 
1. switch по req.URL.Path 
2. Если найден url: 
    - проверка HTTP-метода 
    - вызов соответствующего handler 
3. default -> 404 

Порядок ошибок соответствует ТЗ: наличие метода (404), проверка HTTP метода (405/406), авторизация валидация параметров.
