...
Основная идея решения - построить потоковую обработку данных, где каждая стадия начинает работу сразу после получения значения, не дожидаясь завершения всей предыдущей стадии.

Особое внимание уделяю:
- корректному закрытию каналов
- параллельному вычислению хешей
- предотвращению гонок данных при вызове md5Mutex

### Структура конвейера

Pipeline состоит из нескольких шагов:
1. ExecutePipeline — связывает функции-воркеры в цепочку и управляет каналами
2. SingleHash — считает хеш от входных данных
3. MultiHash — считает несколько хешей параллельно
4. CombineResults — собирает и сортирует финальный результат

#### Функция ExecutePipeline
- создаёт каналы между стадиями
- запускает каждую стадию в отдельной горутине
- закрывает выходной канал после завершения стадии
- дожидается окончания работы всех стадий

Каналы закрываются только здесь, чтобы корректно завершались range по каналам в следующих стадиях.

#### SingleHash
Для каждого входного значения считается:
```
crc32(data) + "~" + crc32(md5(data))
```

Особенности реализации:
- md5 может вызываться только последовательно (ограничение задания)
- crc32 считается параллельно
- вычисления для разных входных значений выполняются конкурентно

Для защиты md5 используется sync.Mutex.

#### MultiHash
Для каждого входного значения считается 6 хешей:
```
crc32("0"+data)
crc32("1"+data)
...
...
Все 6 вычислений запускаются параллельно, а результат собирается в фиксированном порядке

#### CombineResults
Финальная стадия:
- собирает все результаты
- сортирует их
- объединяет в одну строку через _

Это единственная стадия, где допускается накопление всех данных.
